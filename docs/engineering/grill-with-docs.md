...
What it never writes is behaviour. Requirements belong to the change proposal, which is [to-spec](https://aihero.dev/skills-to-spec)'s job downstream.
...
A grilling session can touch your docs in exactly three ways, and the third is the one people expect to be looser than it is:

1. Update `CONTEXT.md` freely — it's living.
2. Create a new ADR, 1-3 sentences, when the decision clears the gate.
3. Change an existing ADR's `Status` — to `deprecated`, or `superseded by ADR-NNNN`.

That's all. The content of an accepted ADR is frozen. If the grilling reverses a past decision — and grillings do that, it's the point — you get a *new* ADR that supersedes the old one, not an edited old one. The record of what you believed at the time survives being wrong.
...
- Reversing a past decision produces a new ADR; `git diff` shows the old one's `Status` line changed and nothing else.
