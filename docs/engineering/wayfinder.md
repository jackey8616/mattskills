...
| A cleared wayfinder map | `/wayfinder` once more to **close the map**, then [to-spec](https://aihero.dev/skills-to-spec), [to-tickets](https://aihero.dev/skills-to-tickets) and [implement](https://aihero.dev/skills-implement) |
...
No. Wayfinder's tickets are decision tickets, and by the time the map closes they are all closed too. What is left is a map full of linked decisions, which is not a build plan. The collapse happens in one last `/wayfinder` session — **Close the map** — which gathers what the closed tickets agreed, what they rejected, and what the map ruled out of scope, then pushes all of it through [writing-proposals](https://aihero.dev/skills-writing-proposals) into the change's `proposal.md` and `design.md`. [to-spec](https://aihero.dev/skills-to-spec) reads that proposal — not the map — and [to-tickets](https://aihero.dev/skills-to-tickets) slices the result into tracer-bullet implementation tickets. Looping the map straight into [implement](https://aihero.dev/skills-implement) skips the collapse and throws the linked detail away. Go straight to implementation only when the effort turned out genuinely small. People do run the abbreviated pipeline and report it working; the extra steps buy you an explicit proposal a reviewer or a colleague can read, which matters more the less solo you are.
...
- Terms land in `CONTEXT.md` as they resolve, not in a batch at the end — and the rare resolution that earns an ADR is linked from its map line, so you can see which ones were promoted.
- The closing session refuses to run while **Not yet specified** still has anything in it.
...
- The session that finishes the map hands you a proposal, not a pull request.

## Where it fits

`wayfinder` is a **situational on-ramp**, not the default front door. The grill-led idea → ship chain is still where most work starts; wayfinder is what you climb onto when the idea is too big to hold in one session, and it merges back onto that chain through [writing-proposals](https://aihero.dev/skills-writing-proposals) — the same closer [grill-with-docs](https://aihero.dev/skills-grill-with-docs) uses — and on into [to-spec](https://aihero.dev/skills-to-spec), because a cleared map hands off rather than builds.
