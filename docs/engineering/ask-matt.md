...
- **The main flow**, idea to ship. Grill, spec, tickets, implement, review, then the close that archives the change into current behaviour — with two branches inside it: a prototype detour when a question needs runnable code to settle, and the spec-and-tickets split, which only earns its cost when the build spans more than one session.
