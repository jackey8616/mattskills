...
4. **Grill (if needed).** If the request needs fleshing out, run the `/grilling` and `/domain-modeling` skills together — grill it into shape one question at a time, sharpening domain terms in `CONTEXT.md` as decisions land. `/domain-modeling` governs what may be written: new ADRs only, never an edit to an accepted one.
